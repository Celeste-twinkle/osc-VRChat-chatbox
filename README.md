...
- AI 模式可开启“翻译前纠正原文”。模型会在一次请求中谨慎纠正明确的错别字、拼写或语法问题并翻译，尽量保留原意、语气、专名、俚语和换行；“原文 + 译文”使用纠正后的原文。如果兼容 API 不按约定返回结构化结果，页面会自动退回普通翻译结果。
...
“翻译前纠正原文”只用于 AI 提供商，不会对 MyMemory 生效。它不会增加第二次 API 调用；页面会在现有系统提示词后追加严格的 JSON 输出约定，并兼容带 Markdown 代码块或退回纯译文的模型。纯数字、纯符号和 Emoji 等输入仍会在浏览器中直接短路，不调用任何翻译 API。
